...
class ATCommands {
 private:
  ...
  uint16_t bufferPos = 0;  // keeps track of the buffer so that we don't
                           // overflow based on bufferSize above
  const char *term;
  ...
  bool parseParameters(
      uint16_t pos);      // parses parameters in the case of a WRITE command
 ...
 public:
   ...
   * command handler.
   * @return SERIAL_COMMANDS_SUCCESS when successful or
   * SERIAL_COMMANDS_ERROR_XXXX on error.
   */
  AT_COMMANDS_ERRORS update();

  AT_COMMANDS_ERRORS manual_update(const char *command);
