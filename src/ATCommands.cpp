...
void ATCommands::begin(Stream *stream, const at_command_t *commands,
                       uint32_t size, const uint16_t bufferSize,
...
bool ATCommands::parseParameters(uint16_t pos) {
  this->bufferString =
      this->bufferString.substring(pos + 1, this->bufferSize - pos + 1);
 ...
 * this goes in a loop but it is expected the user knows their own parameters so
 * there would be no need to exceed boundaries.
 * @return char*
 */
 ...
 * to the buffer.  When the line terminator is found continues to parse and
 * eventually process the command.
 ...
 * Invokes the defined handler to process (callback) the command on the user's
 * side.
 */
